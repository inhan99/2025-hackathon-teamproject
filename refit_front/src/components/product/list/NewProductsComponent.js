...
const NewProductsComponent = () => {
  ...
  useEffect(() => {
    ...
    reset(); // 이걸 추가해야 새 카테고리 선택 시 데이터 초기화되고 재호출됨
    // 초기 스크롤 위치 체크 - 지연 시간을 늘려서 스크롤바 렌더링 완료 후 체크
    setTimeout(checkScrollPosition, 200);
    // 추가로 한 번 더 체크하여 확실히 처리
  ...
  return (
    <section
      className="py-4 bg-[#ffffff] border-l-4 border-r-4 relative"
    ...
    >
      <div className="max-w-[1440px] mx-auto relative">
        <h2 className="text-2xl font-bold mb-6 text-purple-600 px-6">
          New Items <br />
          <p className="text-gray-600">신규 출시 아이템</p>
        </h2>
        {/* 카테고리 버튼 */}
        <div className="flex gap-2 mb-6 px-6">
          ...
          {categories.map(({ label, mainCategoryId, subCategoryId }) => (
            <button
              ...
              onClick={() => {
                console.log("버튼 클릭", mainCategoryId, subCategoryId);
